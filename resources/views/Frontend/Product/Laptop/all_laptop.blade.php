...
foreach ($product as $productSubid) {
    $subid = $productSubid->category_id;
}
@endphp
@if ( $isempty != 1)
...
<script>
    ...
    $('#search_minmax').click(function () {
        // Get filter values
        var brands = $('#brands :selected').val();
        var sort = $('#sort :selected').val();
        var min = $('#min').val();
        var max = $('#max').val();
        var link = "{{ route('Accessories.Monitor') }}" + "?brands=" + brands + "&min=" + min + "&max=" + max + "&sort=" + sort;
